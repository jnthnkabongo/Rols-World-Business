...
@section('content')
    ...
    <main class="p-4 md:p-8 overflow-y-auto flex-1">
            ...
            <div class="bg-gradient-to-br from-green-400 to-green-600 p-4 md:p-6 rounded-2xl shadow-lg card-hover transition-all duration-300 transform hover:scale-105">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-xl md:text-3xl font-bold text-white mt-1">{{ number_format($benefice_total, 2, ',', ' ') }} $</p>
                    </div>
            ...
            <div class="bg-gradient-to-br from-blue-400 to-blue-600 p-4 md:p-6 rounded-2xl shadow-lg card-hover transition-all duration-300 transform hover:scale-105">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-xl md:text-3xl font-bold text-white mt-1">{{ number_format($somme_ventes, 2, ',', ' ') }} $</p>
                    </div>
                    <div class="w-12 h-12 md:w-16 md:h-16 bg-white bg-opacity-20 rounded-2xl flex items-center justify-center backdrop-blur-sm">
            ...
            <div class="bg-gradient-to-br from-orange-400 to-orange-600 p-4 md:p-6 rounded-2xl shadow-lg card-hover transition-all duration-300 transform hover:scale-105">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-xl md:text-3xl font-bold text-white mt-1">{{ number_format($benefice_moyen, 2, ',', ' ') }} $</p>
                    </div>
                    <div class="w-12 h-12 md:w-16 md:h-16 bg-white bg-opacity-20 rounded-2xl flex items-center justify-center backdrop-blur-sm">
            ...
            <div class="bg-gradient-to-br from-amber-400 to-amber-600 p-4 md:p-6 rounded-2xl shadow-lg card-hover transition-all duration-300 transform hover:scale-105">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-xl md:text-3xl font-bold text-white mt-1">{{ $meilleur_produit->nom ?? '-' }}</p>
                    </div>
                    <div class="w-12 h-12 md:w-16 md:h-16 bg-white bg-opacity-20 rounded-2xl flex items-center justify-center backdrop-blur-sm">
        ...
        <div class="bg-white rounded-2xl shadow-lg p-2 md:p-4 mb-4 md:mb-8">
            ...
            <form action="{{ route('liste-rapports') }}" method="GET" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-5">
                <div>
                ...
                <div class="flex items-end">
                    <button type="submit" class="w-full bg-gradient-to-r from-blue-600 to-blue-600 text-white py-2 px-4 rounded-lg hover:from-blue-700 hover:to-blue-700 transition-all duration-300">
                ...
                <table class="w-full">
                    ...
                    <tbody class="divide-y divide-gray-200">
                        ...
                        <tr class="hover:bg-blue-50 transition">
                            ...
                            <td class="px-3 md:px-6 py-3 md:py-4 whitespace-nowrap text-xs md:text-sm text-gray-700">{{ number_format($prix_achat, 2, ',', ' ') }} $</td>
                            <td class="px-3 md:px-6 py-3 md:py-4 whitespace-nowrap text-xs md:text-sm text-gray-700">{{ number_format($prix_vente_unitaire, 2, ',', ' ') }} $</td>
                            <td class="px-3 md:px-6 py-3 md:py-4 whitespace-nowrap text-xs md:text-sm text-gray-700">{{ $detail->quantite ?? '-' }}</td>
                            <td class="px-3 md:px-6 py-3 md:py-4 whitespace-nowrap text-xs md:text-sm text-gray-700">{{ number_format($cout_total, 2, ',', ' ') }} $</td>
                            <td class="px-3 md:px-6 py-3 md:py-4 whitespace-nowrap text-xs md:text-sm font-semibold {{ $benefice >= 0 ? 'text-green-600' : 'text-red-600' }}">{{ number_format($benefice, 2, ',', ' ') }} $</td>
                        ...
                        @empty
                        <tr>
                            <td colspan="8" class="px-3 md:px-6 py-3 md:py-4 text-center text-xs md:text-sm text-gray-500">Aucune vente trouvée</td>
                        </tr>
                        @endforelse
                    </tbody>
                </table>
